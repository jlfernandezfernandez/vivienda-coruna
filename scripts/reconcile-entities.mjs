...
const rejected = rejectedPromotions;
...
const rejectedOpportunityRows = rejectedOpportunities;
...
for (const [id, reason] of rejectedOpportunityRows) {
...
console.log(`Reconciliadas ${aliases.length} variantes de promoción y ${opportunityAliases.length} señales; rechazados ${rejected.length + rejectedOpportunityRows.length} falsos positivos con decisión persistente.`);
